...
4. Click `Show more settings...` to set any [Environmental Variables](../../home/environmental) you wish to use. **None are required for Plex Meta Manager to work.**
...
3. Edit the `Time to Run` variable to reflect a time that is NOT the current time. We don't want Plex Meta Manager to be running right now. Set `Run` to `false` if you've chosen to add that variable. Then, start the container.
...
6. Run `ls` to make sure you're in the same directory as `plex_meta_manager.py`. If you don't see Plex Meta Manager, run `cd /`

7. Run `python plex_meta_manager.py -r`, and watch as Plex Meta Manager comes to life.

8. You'll now notice, as per the [configuration documentation](../../config/configuration) on these sources, Plex Meta Manager will ask you to click a URL and return an input. Go ahead and do so in this terminal window.

9. Once finished, and Plex Meta Manager succeeds in connecting to your source, press `Ctrl + C` to cancel Plex Meta Manager - and close out of the terminal window. Go ahead and stop the docker container, restore your container settings to your original preferences (restore `Time to Run`), and start the container.
